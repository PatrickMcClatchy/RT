...
# Set paths
simulation_path <- "/Users/patrick/Desktop/MASTER THESIS/PAR_SENSOR_SIMULATION_EXPLICIT_FINAL"
sensor_metadata_path <- "/Users/patrick/Desktop/MASTER THESIS/label_x_y_z_pcrs.csv"
validation_data_path <- "/Users/patrick/Desktop/MASTER THESIS/data_combined_all.csv"
...
val_dat <- read_csv(validation_data_path) %>%
  ...
  dplyr::select(logger, sensor, time_floor_utc, measured) %>%
  rename(time = "time_floor_utc") %>%
  distinct() %>%
...
plot_nadir_images(nadir_image_paths, 100, "Nadir Radiance Images")
...
for (sensor in sensor_names) {
  sensor_image_data <- plot_sensor_data(sensor, par_sensor_image_paths, size = 100)
...
mvsi$time %>% na.omit() %>% range()

# Join data and save to CSV
df_join <- full_join(val_dat, mvsi, by = c("logger", "sensor", "time")) %>%
...
df_join_wide <- full_join(val_dat, mvsi, by = c("logger", "sensor", "time")) %>% 
  mutate(simulated = convert_irradiance_to_photon_flux(simulated))

plot(df_join_wide$measured,df_join_wide$simulated )
################################################################################
# Set the fraction of rows to retain
retain_fraction <- 0.3 # Adjust this fraction as needed

# Split the dataframe into rows that meet the condition and those that don't
df_meet_condition <- df_join_wide %>% 
  filter(simulated > 1000 & measured < 500)

df_not_meet_condition <- df_join_wide %>% 
  filter(!(simulated > 1000 & measured < 500))

# Sample a fraction of rows that meet the condition
df_sampled <- df_meet_condition %>% 
  sample_frac(retain_fraction)

df_join_wide <- bind_rows(df_sampled, df_not_meet_condition)
################################################################################
plot(df_join_wide$measured,df_join_wide$simulated )


write_csv(df_join, file.path(out_folder, "DART_PAR_VAL_long_explicit.csv"))
write_csv(df_join_wide, file.path(out_folder,"DART_PAR_VAL_wide_explicit.csv"))
